...
def create_scheduler(context: AppContext, *, blocking: bool = True):
    """Build a scheduler with the scan job registered."""
...
def run_forever(context: AppContext) -> None:
    ...
    settings = context.settings
    scheduler = create_scheduler(context, blocking=True)
    log.info(
        "scheduler_starting",
